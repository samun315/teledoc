    ...
    <div class="post d-flex flex-column-fluid" id="kt_post">
        ...
        <div id="kt_content_container" class="container-fluid">
            ...
            <div class="card">
                ...
                <div class="row p-3 mb-5">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control form-control"
                            value="{{ $doctorInfo?->title }} {{ $doctorInfo?->name }}" disabled>
                    </div>
                    <div class="col-md-6">
                        <div class="card shadow-sm text-center p-3">
                            <img src="{{ $doctorInfo?->photo
                                ? asset('uploads/doctor/' . $doctorInfo?->photo)
                                : asset('assets/media/avatars/blank.png') }}"
                                class="rounded-circle mb-2 mx-auto d-block" width="100" height="100"
                                alt="Doctor Avatar">

                            <h5 class="fw-bold mb-1">{{ $doctorInfo?->title }}
                                {{ $doctorInfo?->name }}</h5>
                            <p class="mb-1"><strong>Department :</strong>
                                {{ $doctorInfo?->department?->department_name ?? 'N/A' }}</p>
                            <p class="mb-1"><strong>Phone :</strong>{{ $doctorInfo?->phone }}</p>
                            <p class="mb-1"><strong>Email: </strong>{{ $doctorInfo?->email }}</p>
                            <p class="mb-0"><strong>Address :</strong>{{ $doctorInfo?->address }}</p>
                        </div>
                    </div>
                ...
                <form method="POST" action="{{ route('schedule.update') }}" id="scheduleForm">
                    @csrf

                    @isset($editModeData)
                        @method('PUT')
                    @endisset
                    <input type="hidden" name="doctor_id" class="form-control form-control"
                    ...
                    <div class="row p-3">
                        ...
                        <div class="table-responsive">
                            <table class="table table-row-dashed align-middle gy-4">
                                ...
                                <tbody id="daysTable">
                                    @if (!empty($editModeData))
                                        @foreach ($editModeData as $schedule)
                                            <tr>
                                                <td>
                                                    <input type="text" hidden value="{{ $schedule?->schedule_id }}"
                                                        name="schedule_id[]">

                                                    <select name="day_of_week[]"
                                                        class="form-select form-select-light dayOfWeek"
                                                ...
                                                <td>
                                                    <button type="button"
                                                        class="btn btn-danger btn-sm removeScheduleRow"><i
                            ...
                            <button type="button" id="addMore" class="btn btn-success btn-sm mt-2">Add More</button>
                        </div>
                    </div>
                    ...
                    <div class="p-5 text-end">
                        <button type="submit" class="btn btn-primary btn-sm text-nowrap" id="btnScheduleSubmit">Update
                            Schedule</button>
                    </div>
                </form>
            </div>
    ...
    <script nonce="{{ $cspNonce }}">
        ...
        $(document).ready(function() {
            ...
            $("#addMore").click(function() {
                ...
                <tr>
                    <td>
                        <input type="text" hidden value="" name="schedule_id[]">
