...
<!--                        <span class="material-icons dnl-grab-icon">menu</span>-->
                        ${
                            config.indexing
                                ? `
                            <span class="material-icons dnl-grab-icon">
                                ${oldLiHtml}
                            </span>
                        `
                                : oldLiHtml
        ...
        this.topLvlUl.find("ul").each((i, e) => {
            ...
            parentLi
                .children(dnlConsts.classes.titleSpan.clas())
                .first()
                .append(
                    `<span class="material-icons dnl-icon-collapsed"><i class="fas fa-angle-right"></i></span><span class="material-icons dnl-icon-expanded"><i class="fas fa-angle-down"></i></span>`
                );
        });
        ...
        $("body").on("mouseup", (e) => {
            ...
            if ($(".potentialNewSpotLi").length) {
                ...
                } else PotentialNewSpotLi.after(RealLi[0]);
    ...
    // Method to get the current order of items
    serialize() {
        const serialized = this.serializeList(this.topLvlUl);
        return serialized;
    }
    ...
    serializeList(ul) {
        ...
        ul.children("li").each((i, li) => {
            const id = $(li).data("id");
            const children = $(li).children("ul").length
                ? this.serializeList($(li).children("ul").first())
                : [];
            serialized.push({ id, children });
        });
        return serialized;
    }
}
