    ...
    public function getPatientList(Request $request): JsonResponse
    {
        ...
        return Datatables::of($query)
            ...
            ->addColumn('photo', function ($row) {
                ...
                $info = '<i class="fas fa-user-injured"></i> : ' . $name . '<br>' . '<i class="fas fa-venus-mars"></i> : ' . $gender . '<br>' . '<i class="fas fa-ring"></i> : ' . $marital_status . '<br>' . '<i class="fas fa-calendar-day"></i> : ' . $dob;
                return $info;
            })->addColumn('contact_info', function ($row) {
                $email = $row->email ?? '';
                $phone = $row->phone ?? '';
                $address = $row->address ?? '';
                $contact_info = '<i class="fas fa-envelope"></i> : ' . $email . '<br>' . '<i class="fas fa-phone"></i> : ' . $phone . '<br>' . '<i class="fas fa-map-marked-alt"></i> : ' . $address;
                return $contact_info;
            })->addColumn('medical_info', function ($row) {
                $height = $row->height ?? '';
                $weight = $row->weight ?? '';
                $blood_group = $row->blood_group ?? '';
                $medical_info = '<i class="fas fa-ruler-vertical"></i> : ' . $height . '<br>' . '<i class="fas fa-weight"></i> : ' . $weight . '<br>' . '<i class="fas fa-tint"></i> : ' . $blood_group;
                return $medical_info;
